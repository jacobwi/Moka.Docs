...
public sealed class BuildPipelineIntegrationTests
{
    ...
    public async Task FullPipeline_WithMarkdownFiles_ProducesOutput()
    {
        ...
        var (pipeline, _) = CreatePipeline(fs);
        ...
        var homeHtml = fs.File.ReadAllText("/project/_site/index.html");
        homeHtml.Should().Contain("Welcome");
        homeHtml.Should().Contain("<h1");

        var guideHtml = fs.File.ReadAllText("/project/_site/guide/getting-started/index.html");
        ...
        var sitemap = fs.File.ReadAllText("/project/_site/sitemap.xml");
