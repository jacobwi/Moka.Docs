...
public sealed class NuGetPackageResolver(ILogger logger)
{
        ...
        CancellationToken ct = default)
    {
        ...
        var tempDir = Path.Combine(Path.GetTempPath(), "mokadocs-repl-" + Guid.NewGuid().ToString("N")[..8]);
        Directory.CreateDirectory(tempDir);

        try
        {
            var csproj = BuildProjectFile(packageSpecs);
            var projectPath = Path.Combine(tempDir, "ReplPackages.csproj");
            ...
            foreach (var dll in Directory.EnumerateFiles(publishDir, "*.dll"))
            {
                ...
                try
                {
                    ...
                    foreach (var ns in GetRootNamespaces(asm))
                        namespaces.Add(ns);
    ...
    private static string BuildProjectFile(IReadOnlyList<string> packageSpecs)
    {
        ...
        {
            var (name, version) = ParseSpec(spec);
