...
public sealed class DevServer : IDisposable
{
    ...
    private async Task HandleRequestAsync(HttpListenerContext context, CancellationToken ct)
    {
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            try
            {
                ...
                response.ContentType = "text/plain";
                var msg = Encoding.UTF8.GetBytes("500 Internal Server Error");
                await response.OutputStream.WriteAsync(msg, ct);
            }
            catch
            {
        ...
        CancellationToken ct)
    {
        ...
        if (_replService is null)
        {
            ...
            var errorJson = JsonSerializer.Serialize(new { error = "REPL service is not available." });
        ...
        try
        {
            ...
            var payload = JsonSerializer.Deserialize<ReplRequest>(body, _jsonOptions);
            var code = payload?.Code ?? "";

            var result = await _replService.ExecuteAsync(code, ct);

            response.StatusCode = 200;
            var json = JsonSerializer.Serialize(new { output = result.Output ?? "", error = result.Error ?? "" },
                _jsonOptions);
        ...
        catch (Exception ex)
        {
            ...
            response.StatusCode = 500;
            var json = JsonSerializer.Serialize(new { error = $"Internal error: {ex.Message}" }, _jsonOptions);
            var bytes = Encoding.UTF8.GetBytes(json);
            response.ContentLength64 = bytes.Length;
            await response.OutputStream.WriteAsync(bytes, ct);
        }
        ...
        CancellationToken ct)
    {
        ...
        {
            response.StatusCode = 503;
            var errorJson = JsonSerializer.Serialize(new { error = "Blazor preview service is not available." });
            var errorBytes = Encoding.UTF8.GetBytes(errorJson);
            response.ContentLength64 = errorBytes.Length;
            await response.OutputStream.WriteAsync(errorBytes, ct);
            return;
        ...
        try
        {
            using var reader = new StreamReader(request.InputStream, Encoding.UTF8);
            var body = await reader.ReadToEndAsync(ct);
            var payload = JsonSerializer.Deserialize<BlazorPreviewRequest>(body, _jsonOptions);
            var source = payload?.Source ?? "";

            var result = await _blazorPreviewService.RenderAsync(source, ct);

            response.StatusCode = 200;
            var json = JsonSerializer.Serialize(new { html = result.Html ?? "", error = result.Error ?? "" },
                _jsonOptions);
            var bytes = Encoding.UTF8.GetBytes(json);
            response.ContentLength64 = bytes.Length;
            await response.OutputStream.WriteAsync(bytes, ct);
        ...
        CancellationToken ct)
    {
        ...
        try
        {
            using var reader = new StreamReader(request.InputStream, Encoding.UTF8);
            var body = await reader.ReadToEndAsync(ct);
            var payload = JsonSerializer.Deserialize<FeedbackRequest>(body, _jsonOptions);

            var page = payload?.Page ?? "(unknown)";
            var helpful = payload?.Helpful;
            var emoji = helpful == true ? "👍" : "👎";
    ...
    private string? ResolveFilePath(string urlPath)
    {
        ...
        var filePath = Path.Combine(_rootPath, decoded.Replace('/', Path.DirectorySeparatorChar));
        ...
        if (!File.Exists(filePath) && !Path.HasExtension(filePath)) filePath += ".html";
