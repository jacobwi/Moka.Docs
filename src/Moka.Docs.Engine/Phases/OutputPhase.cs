...
public sealed class OutputPhase(ILogger<OutputPhase> logger) : IBuildPhase
{
    ...
    public Task ExecuteAsync(BuildContext context, CancellationToken ct = default)
    {
        ...
        foreach (var page in context.Pages)
        {
            ...
            var pagePath = RouteToFilePath(page.Route);
            var fullPath = fs.Path.Combine(outputDir, pagePath);
        ...
        var redirectCount = GenerateSectionIndexPages(context);
        ...
        // Copy static assets
        var assetsCopied = CopyAssets(context);
    ...
    {
        var fs = context.FileSystem;
        var docsPath = fs.Path.GetFullPath(
            fs.Path.Combine(context.RootDirectory, context.Config.Content.Docs));
        var outputDir = context.OutputDirectory;
        var count = 0;

        foreach (var assetPath in context.DiscoveredAssetFiles)
        {
            var sourcePath = fs.Path.Combine(docsPath, assetPath);
            var destPath = fs.Path.Combine(outputDir, assetPath);
            var destDir = fs.Path.GetDirectoryName(destPath)!;

            if (!fs.File.Exists(sourcePath)) continue;
    ...
    private void WriteSearchIndex(BuildContext context)
    {
        ...
        var json = JsonSerializer.Serialize(entries, SearchJsonOptions);
    ...
    private int GenerateSectionIndexPages(BuildContext context)
    {
        ...
        foreach (var page in context.Pages)
        {
            ...
            var filePath = RouteToFilePath(page.Route);
            var fullPath = fs.Path.Combine(outputDir, filePath);
            var dir = fs.Path.GetDirectoryName(fullPath)!;
        ...
        foreach (var dir in dirsWithIndex)
        {
            ...
            var parent = fs.Path.GetDirectoryName(dir);
            while (parent != null
                   ...
                   && parent.StartsWith(outputDir, StringComparison.OrdinalIgnoreCase))
            {
                ...
                var parentIndex = fs.Path.Combine(parent, "index.html");
                ...
                {
                    var childIndex = fs.Path.Combine(child, "index.html");
                ...
                var relativePath = parentIndex[outputDir.Length..].TrimStart(
        ...
        if (count > 0) logger.LogInformation("Generated {Count} section index redirect(s)", count);
