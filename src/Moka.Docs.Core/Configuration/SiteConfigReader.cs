...
public sealed class SiteConfigReader
{
    ...
    public SiteConfig Read(string configPath)
    {
        ...
        var yaml = _fileSystem.File.ReadAllText(configPath);
        return Parse(yaml);
    }
    ...
    public static SiteConfig Parse(string yaml)
    {
        try
        {
            ...
            var dto = deserializer.Deserialize<SiteConfigDto>(yaml)
    ...
    {
        var siteDto = dto.Site ??
                      throw new SiteConfigException("'site' section is required and must include a 'title'.");
        if (string.IsNullOrWhiteSpace(siteDto.Title)) throw new SiteConfigException("'site.title' is required.");
    ...
    {
        if (dto is null) return new ContentConfig();
    ...
    {
        if (dto is null) return new ThemeConfig();
    ...
    {
        if (dto is null) return new FeaturesConfig();
    ...
    {
        if (dto is null) return new CloudConfig();
    ...
    {
        if (dto is null) return new BuildConfig();
