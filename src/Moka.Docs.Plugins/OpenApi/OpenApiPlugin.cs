...
public sealed class OpenApiPlugin : IMokaPlugin
{
    ...
    public Task InitializeAsync(IPluginContext context, CancellationToken ct = default)
    {
        ...
        if (options.TryGetValue("spec", out var specObj) && specObj is string specStr)
            _specPath = specStr;

        if (options.TryGetValue("label", out var labelObj) && labelObj is string labelStr)
            _label = labelStr;

        if (options.TryGetValue("routePrefix", out var prefixObj) && prefixObj is string prefixStr)
    ...
    {
        var specFullPath = ResolveSpecPath(buildContext);
        ...
        var pages = GeneratePages(spec, context);
    ...
    {
        var root = buildContext.RootDirectory;

        // User-specified path
        if (!string.IsNullOrWhiteSpace(_specPath))
        {
            var full = Path.IsPathRooted(_specPath)
                ? _specPath
    ...
    private List<DocPage> GeneratePages(OpenApiSpec spec, IPluginContext context)
    {
        ...
        var inlineCss = OpenApiPageRenderer.GetInlineCss();
        ...
        {
            var tags = ep.Tags.Count > 0 ? ep.Tags : ["Other"];
        ...
        // Index page
        var indexHtml = inlineCss + OpenApiPageRenderer.RenderIndexPage(spec, _routePrefix);
        ...
        foreach (var (tag, endpoints) in grouped.OrderBy(kv => kv.Key))
        {
            ...
            var tagHtml = inlineCss + OpenApiPageRenderer.RenderTagPage(tag, endpoints);

            var plainTextParts = new List<string> { tag };
            foreach (var ep in endpoints) plainTextParts.Add($"{ep.Method} {ep.Path} {ep.Summary}");
