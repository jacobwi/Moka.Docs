...
public sealed class PluginHost
{
    ...
    {
        var declarations = _siteConfig.Plugins;
        ...
        var available = _serviceProvider.GetService(typeof(IEnumerable<IMokaPlugin>))
            as IEnumerable<IMokaPlugin> ?? [];

        var pluginMap = new Dictionary<string, IMokaPlugin>(StringComparer.OrdinalIgnoreCase);
        foreach (var plugin in available) pluginMap[plugin.Id] = plugin;
        ...
        foreach (var declaration in declarations)
        {
            ...
            if (!pluginMap.TryGetValue(declaration.Name, out var plugin))
            {
            ...
            var pluginLogger = _serviceProvider.GetService(typeof(ILogger<PluginHost>)) as ILogger
                               ?? _logger;
