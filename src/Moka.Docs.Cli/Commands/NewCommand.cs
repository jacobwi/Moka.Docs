...
internal static class NewCommand
{
    ...
    private static Command CreatePageCommand()
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            var title = parseResult.GetValue(titleOpt) ?? ToTitleCase(name);
            var outputDir = parseResult.GetValue(pathOpt)!;
            var layout = parseResult.GetValue(layoutOpt)!;
            var order = parseResult.GetValue(orderOpt);
            ...
            var filePath = Path.GetFullPath(Path.Combine(outputDir, $"{name}.md"));
    ...
    private static Command CreatePluginCommand()
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            var projectName = $"Moka.Docs.Plugins.{pascal}";
            var projectDir = Path.GetFullPath(Path.Combine(outputDir, projectName));
    ...
    private static Command CreateComponentCommand()
    {
        ...
        command.SetAction(parseResult =>
        {
            var name = parseResult.GetValue(nameArg)!.ToLowerInvariant();
            var outputDir = parseResult.GetValue(pathOpt)!;

            AnsiConsole.MarkupLine("[bold green]mokadocs new component[/] — Scaffolding component example...");

            if (!ComponentTemplates.TryGetValue(name, out var template))
            {
                AnsiConsole.MarkupLine(
            ...
            var filePath = Path.GetFullPath(Path.Combine(outputDir, $"{name}-example.md"));
            if (File.Exists(filePath))
            {
                AnsiConsole.MarkupLine($"[yellow]{filePath} already exists. Skipping.[/]");
                return 0;
