...
internal static class StatsCommand
{
    ...
    public static Command Create()
    {
        ...
        command.SetAction(async (parseResult, _) =>
        {
            ...
            var markdownFiles = Directory.Exists(docsDir)
                ? Directory.GetFiles(docsDir, "*.md", SearchOption.AllDirectories)
                : [];

            var markdownPageCount = markdownFiles.Length;
            ...
            if (apiTypes > 0)
            {
                ...
                var coverage = apiTypes > 0 ? typesWithSummary * 100 / apiTypes : 100;
            ...
            {
                var allDocsFiles = Directory.GetFiles(docsDir, "*.*", SearchOption.AllDirectories);
                long totalSize = 0;
                foreach (var f in allDocsFiles)
            ...
            if (outputJson)
            {
                var jsonOutput = JsonSerializer.Serialize(stats, new JsonSerializerOptions
                {
                    WriteIndented = true
                });
            ...
            else
            {
                ...
                if (apiTypes > 0)
                {
                    ...
                    var coverage = apiTypes > 0 ? typesWithSummary * 100 / apiTypes : 100;
                    var coverageColor = coverage >= 90 ? "green" : coverage >= 50 ? "yellow" : "red";
