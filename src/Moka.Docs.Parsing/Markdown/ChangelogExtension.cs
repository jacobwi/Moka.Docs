...
public sealed class ChangelogParser : BlockParser
{
    ...
    {
        if (block is not ChangelogBlock changelogBlock) return BlockState.None;
...
public sealed class ChangelogRenderer : HtmlObjectRenderer<ChangelogBlock>
{
    ...
    {
        var entries = ParseEntries(block.RawLines);

        renderer.EnsureLine();
        renderer.Write("<div class=\"changelog\">");
        renderer.WriteLine();

        foreach (var entry in entries) WriteEntry(renderer, entry);
    ...
    private static List<ChangelogEntry> ParseEntries(List<string> lines)
    {
        ...
        foreach (var rawLine in lines)
        {
            ...
            var typeMatch = TypeLineRegex.Match(line);
            ...
            if (itemMatch.Success && currentCategory != null) currentCategory.Items.Add(itemMatch.Groups[1].Value);
        }
    ...
    {
        var escapedVersion = HttpUtility.HtmlAttributeEncode(entry.Version);
        var escapedType = HttpUtility.HtmlAttributeEncode(entry.Type);
        ...
        // Categories
        foreach (var category in entry.Categories) WriteCategory(renderer, category);
    ...
    {
        var info = CategoryInfo.GetValueOrDefault(category.Name);
        var cssClass = info.CssClass ?? "changelog-other";
        var icon = info.Icon ?? "\u2022";
        var label = info.Label ?? category.Name;
