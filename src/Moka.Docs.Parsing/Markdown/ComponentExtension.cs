...
public sealed class ComponentParser : BlockParser
{
    ...
    public override BlockState TryOpen(BlockProcessor processor)
    {
        ...
        if (braceIndex > 0)
        {
            ...
            var closeBrace = remaining.LastIndexOf('}');
            if (closeBrace > braceIndex) attrString = remaining[(braceIndex + 1)..closeBrace];
        }
        else
        {
        ...
        if (!ValidTypes.Contains(typeName)) return BlockState.None;
    ...
    {
        if (block is not ContainerBlock) return BlockState.Continue;
    ...
    private static Dictionary<string, string> ParseAttributes(string? attrString)
    {
        ...
        if (string.IsNullOrWhiteSpace(attrString)) return result;

        var span = attrString.AsSpan();
        while (span.Length > 0)
        {
            span = span.TrimStart();
            ...
            if (span.Length > 0 && (span[0] == '"' || span[0] == '\''))
            {
                var quote = span[0];
                span = span[1..];
                var endQuote = span.IndexOf(quote);
                if (endQuote < 0) break;
...
public sealed class CardRenderer : HtmlObjectRenderer<CardBlock>
{
    ...
    protected override void Write(HtmlRenderer renderer, CardBlock block)
    {
        ...
        var variantClass = variant != "default" ? $" component-card-{variant}" : "";
        ...
        if (!string.IsNullOrEmpty(block.Title))
        {
            ...
            if (!string.IsNullOrEmpty(block.Icon) && IconMap.TryGetValue(block.Icon, out var svg))
    ...
    protected override void Write(HtmlRenderer renderer, StepsBlock block)
    {
        ...
        foreach (var child in block)
            if (child is HeadingBlock heading && heading.Level == 3)
            {
                ...
                if (heading.Inline != null) renderer.WriteChildren(heading.Inline);
                renderer.Write("</h3>");
                renderer.WriteLine();
            }
            else
            ...
            if (block is ParagraphBlock para && para.Inline != null)
                foreach (var inline in para.Inline)
                    ...
                    else if (inline is LiteralInline literal)
                    {
                        ...
                        else if (!string.IsNullOrWhiteSpace(text)) description = text;
                    }

        if (string.IsNullOrEmpty(href) || string.IsNullOrEmpty(title)) return;
...
public sealed class CodeGroupRenderer : HtmlObjectRenderer<CodeGroupBlock>
{
    ...
    protected override void Write(HtmlRenderer renderer, CodeGroupBlock block)
    {
        ...
        foreach (var child in block)
            if (child is FencedCodeBlock fenced)
            {
                ...
                var title = ExtractTitle(fenced.Arguments) ?? LanguageDisplayName(info);
    ...
    private static string? ExtractTitle(string? arguments)
    {
        ...
        var idx = arguments.IndexOf(prefix, StringComparison.OrdinalIgnoreCase);
        if (idx < 0) return null;

        var rest = arguments[(idx + prefix.Length)..];
        if (rest.Length < 2) return null;

        var quote = rest[0];
        if (quote != '"' && quote != '\'') return null;

        var endQuote = rest.IndexOf(quote, 1);
