...
public sealed class MarkdownParser
{
    ...
    public MarkdownParseResult Parse(string markdown)
    {
        ...
        var fmResult = _frontMatterExtractor.Extract(markdown);

        // Step 2: Parse Markdown body
        var document = Markdig.Markdown.Parse(fmResult.Body, _pipeline);

        // Step 3: Generate ToC
        var toc = _tocGenerator.Generate(document);
    ...
    private static string ExtractPlainText(MarkdownDocument document)
    {
        ...
        foreach (var block in document.AllDescendants())
            if (block is ParagraphBlock paragraph && paragraph.Inline is not null)
            {
                foreach (var inline in paragraph.Inline) AppendInlineText(sb, inline);
                sb.AppendLine();
            }
            else if (block is HeadingBlock heading && heading.Inline is not null)
            {
                foreach (var inline in heading.Inline) AppendInlineText(sb, inline);
                sb.AppendLine();
            }
    ...
    private static void AppendInlineText(StringBuilder sb, Inline inline)
    {
        switch (inline)
        {
            ...
            case ContainerInline container:
                foreach (var child in container) AppendInlineText(sb, child);
                break;
